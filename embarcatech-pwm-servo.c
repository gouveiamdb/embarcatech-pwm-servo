...
void setup_pwm() {
    ...
    uint led_num = pwm_gpio_to_slice_num(LED_PIN);

    pwm_set_clkdiv(slice_num, CLOCK_DIV);
    pwm_set_wrap(slice_num, WRAP_COUNT);
    
    pwm_set_clkdiv(led_num, CLOCK_DIV);
    pwm_set_wrap(led_num, WRAP_COUNT);

    pwm_set_enabled(slice_num, true);
    pwm_set_enabled(led_num, true);
}
